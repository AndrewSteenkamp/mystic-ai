...
export default function FacePage() {
  ...
  const handleRead = () => {
    ...
    setError("");
    mutate.mutate({ imageBase64: image.split(",")[1] || image });
  };
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold">👤 Face Reading</h1>
  ...
  return (
    <div className="max-w-lg mx-auto">
      <h1 className="text-2xl font-bold mb-2">👤 Face Reading (Mian Xiang)</h1>
      <p className="text-gray-400 mb-6">Upload a clear front-facing photo for an ancient Chinese face reading.</p>
      <div className="space-y-4 mb-6">
        {image ? (
          <div className="relative">
            <img src={image} alt="Face" className="w-full rounded-xl border border-purple-800" />
            <button onClick={() => setImage(null)} className="absolute top-2 right-2 bg-red-600 text-white rounded-full w-6 h-6 text-xs">✕</button>
          </div>
        ) : (
          <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-purple-800 rounded-xl p-12 text-center cursor-pointer hover:border-purple-600">
            <Upload className="w-8 h-8 text-purple-500 mx-auto mb-2" />
            <p className="text-gray-400 text-sm">Click to upload face photo</p>
          </div>
        )}
        ...
        {image && (
          ...
          >
            {mutate.isPending ? (
              <><Loader2 className="w-4 h-4 animate-spin" /> Reading your face...</>
            ) : (
