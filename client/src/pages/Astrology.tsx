...
export default function AstrologyPage() {
  ...
  const [birthTime, setBirthTime] = useState(""); const [birthPlace, setBirthPlace] = useState("");
  const [reading, setReading] = useState<any>(null); const [error, setError] = useState("");
  ...
  const handleRead = () => {
    if (!name || !birthdate.match(/^\d{4}-\d{2}-\d{2}$/)) { setError("Please enter your name and birthdate"); return; }
    setError(""); mutate.mutate({ name, birthdate, birthTime: birthTime || null, birthPlace: birthPlace || null });
  };
  if (reading) {
    const s = reading.signes || reading.signs;
    const big3 = [{ label: "Sun", key: "sun", emoji: "☀️" },{ label: "Moon", key: "moon", emoji: "🌙" },{ label: "Rising", key: "rising", emoji: "⬆️" }];
    const planets = [{ label: "Mercury", key: "mercury", emoji: "☿" },{ label: "Venus", key: "venus", emoji: "♀" },{ label: "Mars", key: "mars", emoji: "♂" }];
    return (
      <div>
        <div className="flex items-center justify-between mb-6"><h1 className="text-2xl font-bold">⭐ Your Birth Chart</h1><button onClick={() => setReading(null)} className="text-sm text-purple-400 hover:text-purple-300 flex items-center gap-1"><RefreshCw className="w-3 h-3" /> New Chart</button></div>
        <div className="grid grid-cols-3 gap-3 mb-4">{big3.map(b => (<div key={b.key} className="glass-card rounded-xl p-4 text-center"><div className="text-sm text-gray-400">{b.emoji} {b.label}</div><div className="text-2xl font-bold text-purple-300 mt-1">{s?.[b.key]?.sign}</div></div>))}</div>
        <div className="grid grid-cols-3 gap-3 mb-6">{planets.map(p => (<div key={p.key} className="glass-card rounded-xl p-3 text-center"><div className="text-xs text-gray-500">{p.emoji} {p.label}</div><div className="text-lg font-semibold text-purple-400">{s?.[p.key]?.sign}</div></div>))}</div>
  ...
  return (
    <div className="max-w-lg mx-auto"><h1 className="text-2xl font-bold mb-2">⭐ Astrology Birth Chart</h1><p className="text-gray-400 mb-6">Your complete cosmic blueprint — Sun, Moon, Rising & planets.</p>
