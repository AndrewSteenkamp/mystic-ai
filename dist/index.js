...
async function initializePaystackTransaction(amount, email, callbackUrl, metadata, currency = "USD") {
  const payload = {
...
async function breakdownRecipe(recipeText) {
...
async function generateWeeklyMenu(preferences) {
...
function getMealPlan(userId) {
  ...
  try {
    ...
    if (!row2) return null;
    return {
      menu: JSON.parse(row2.plan_data),
      shoppingList: JSON.parse(row2.shopping_list),
...
function row(id, name, category, kj, protein, carbs, fat, opts = {}) {
  return {
    id,
    name,
    category,
...
var FOOD_DB = [
  ...
  row("strawberry", "Strawberries (raw)", "produce", 136, 0.7, 7.7, 0.3, { region: "global", fiber: 2, aliases: ["strawberries"] }),
  ...
  row("avocado", "Avocado (raw)", "produce", 670, 2, 8.5, 15, { region: "global", fiber: 6.7, aliases: ["avocados", "avo", "avos", "pear"] }),
  ...
  row("kiwi", "Kiwi Fruit (raw)", "produce", 264, 1.1, 15, 0.5, { region: "global", fiber: 3, aliases: ["kiwifruit", "kiwis"] }),
  ...
  row("grapefruit", "Grapefruit (raw)", "produce", 138, 0.8, 9, 0.1, { region: "global", fiber: 1.6, aliases: ["grapefruits", "pomelo"] }),
  row("cherry", "Cherries (raw)", "produce", 263, 1.1, 16, 0.3, { region: "global", fiber: 2.1, aliases: ["cherries"] }),
  row("pomegranate", "Pomegranate (raw)", "produce", 285, 1.7, 18, 1.2, { region: "global", fiber: 4, aliases: ["pomegranates"] }),
  ...
  row("sweet-potato", "Sweet Potato (boiled)", "produce", 360, 1.6, 20, 0.1, { region: "global", fiber: 3, aliases: ["sweet potato", "yam", "yams"] }),
  row("tomato", "Tomato (raw)", "produce", 74, 0.9, 3.9, 0.2, { region: "global", fiber: 1.2, aliases: ["tomatoes", "tamatar"] }),
  row("onion", "Onion (raw)", "produce", 155, 1.1, 9, 0.1, { region: "global", fiber: 1.7, aliases: ["onions", "red onion", "white onion"] }),
  ...
  row("spinach-cooked", "Spinach (cooked)", "produce", 100, 2.7, 4, 0.2, { region: "global", fiber: 2.4 }),
  ...
  row("broccoli", "Broccoli (raw)", "produce", 141, 2.8, 7, 0.4, { region: "global", fiber: 2.6, aliases: ["brocoli"] }),
  row("cauliflower", "Cauliflower (raw)", "produce", 104, 1.9, 5, 0.3, { region: "global", fiber: 2, aliases: ["cauli"] }),
  row("cucumber", "Cucumber (raw, with skin)", "produce", 65, 0.7, 3.6, 0.1, { region: "global", fiber: 0.5, aliases: ["cukes"] }),
  row("bell-pepper-red", "Bell Pepper, Red (raw)", "produce", 130, 1, 6, 0.3, { region: "global", fiber: 2.1, aliases: ["red pepper", "capsicum red", "sweet pepper"] }),
  ...
  row("bell-pepper-yellow", "Bell Pepper, Yellow (raw)", "produce", 130, 1, 6, 0.3, { region: "global", fiber: 1.7, aliases: ["yellow pepper", "capsicum yellow"] }),
  row("mushroom", "Mushrooms, White (raw)", "produce", 93, 3.1, 3.3, 0.3, { region: "global", fiber: 1, aliases: ["mushrooms", "button mushroom"] }),
  row("zucchini", "Zucchini / Baby Marrow (raw)", "produce", 67, 1.2, 3.1, 0.3, { region: "global", fiber: 1, aliases: ["baby marrow", "courgette", "courgettes"] }),
  row("eggplant", "Eggplant / Brinjal (raw)", "produce", 100, 1, 5.9, 0.2, { region: "global", fiber: 3, aliases: ["brinjal", "aubergine"] }),
  row("green-beans", "Green Beans (raw)", "produce", 132, 1.8, 7, 0.2, { region: "global", fiber: 2.7, aliases: ["beans green", "string beans"] }),
  ...
  row("butternut", "Butternut (cooked)", "produce", 175, 1.4, 9, 0.1, { region: "sa", fiber: 1.8, aliases: ["butternut squash", "butternut pumpkin"] }),
  row("pumpkin", "Pumpkin (cooked)", "produce", 100, 1, 5, 0.1, { region: "global", fiber: 1.1, aliases: ["pumpkin", "boerpampoen"] }),
  row("brussels-sprouts", "Brussels Sprouts (raw)", "produce", 175, 3.4, 9, 0.3, { region: "global", fiber: 3.8, aliases: ["brussel sprouts"] }),
  row("asparagus", "Asparagus (raw)", "produce", 85, 2.2, 4.1, 0.1, { region: "global", fiber: 2.1, aliases: ["asparagas", "spargel"] }),
  row("celery", "Celery (raw)", "produce", 58, 0.7, 3, 0.2, { region: "global", fiber: 1.6, aliases: ["celery stalk"] }),
  row("ginger", "Ginger (raw)", "produce", 335, 1.8, 18, 0.8, { region: "global", fiber: 2, aliases: ["fresh ginger", "ginger root"] }),
  ...
  row("chicken-wing", "Chicken Wing (roasted, skin on)", "protein", 1e3, 22, 0, 14, { region: "global", aliases: ["wings", "chicken wings"] }),
  ...
  row("milk-low-fat", "Milk, Low Fat (2%)", "dairy", 195, 3.4, 4.7, 2, { region: "global", aliases: ["low fat milk", "2 percent milk"] }),
  row("milk-skim", "Milk, Skim / Fat Free", "dairy", 145, 3.4, 5, 0.2, { region: "global", aliases: ["skim milk", "fat free milk"] }),
  row("milk-amasi", "Amasi (Maas / Sour Milk)", "dairy", 290, 3.5, 4.5, 3.5, { region: "sa", aliases: ["maas", "sour milk"] }),
  row("yogurt-greek", "Yogurt, Greek (plain, full fat)", "dairy", 380, 9, 3.6, 5, { region: "global", aliases: ["greek yogurt", "greek yoghurt"] }),
  row("yogurt-plain", "Yogurt, Plain (full fat)", "dairy", 280, 3.5, 4.7, 3.3, { region: "global", aliases: ["yoghurt", "yogurt"] }),
  row("yogurt-low-fat", "Yogurt, Low Fat (plain)", "dairy", 230, 4, 6, 1.5, { region: "global" }),
  row("butter", "Butter", "dairy", 3e3, 0.9, 0.1, 81, { region: "global" }),
  ...
  row("cheese-cream", "Cream Cheese", "dairy", 1500, 6, 4, 34, { region: "global" }),
  row("cream", "Cream, Whipping", "dairy", 1300, 2.5, 3, 33, { region: "global", aliases: ["whipping cream", "thick cream"] }),
  row("cream-sour", "Sour Cream", "dairy", 1100, 2.4, 4, 24, { region: "global" }),
  ...
  row("lentils-cooked", "Lentils (boiled)", "protein", 450, 9, 20, 0.4, { region: "global", fiber: 7.9, aliases: ["lentil", "lentils"] }),
  row("chickpeas-cooked", "Chickpeas (boiled)", "protein", 690, 9, 27, 2.6, { region: "global", fiber: 7.6, aliases: ["chickpea", "garbanzo"] }),
  ...
  row("baked-beans", "Baked Beans (canned in tomato sauce)", "protein", 380, 5, 14, 0.4, { region: "global", aliases: ["beans in tomato sauce"] }),
  ...
  row("macadamia", "Macadamia Nuts (raw)", "protein", 3e3, 8, 14, 76, { region: "global" }),
  ...
  row("rice-brown-cooked", "Rice, Brown (boiled)", "grain", 510, 2.6, 26, 1, { region: "global", fiber: 2.8, aliases: ["brown rice"] }),
  row("rice-basmati", "Rice, Basmati (boiled)", "grain", 540, 3, 28, 0.4, { region: "global", aliases: ["basmati rice"] }),
  row("pap", "Pap (cooked maize meal)", "grain", 380, 3, 18, 0.5, { region: "sa", aliases: ["maize porridge", "mealie meal", "sadza", "ugali"] }),
  row("samp", "Samp (cooked)", "grain", 500, 4, 24, 0.5, { region: "sa" }),
  row("oats-cooked", "Oats, Rolled (cooked with water)", "grain", 380, 3, 16, 1.5, { region: "global", fiber: 2.5, aliases: ["oatmeal", "porridge oats"] }),
  row("bread-white", "Bread, White", "grain", 1050, 9, 49, 3.2, { region: "global", aliases: ["white bread", "toast", "sandwich bread"] }),
  row("bread-whole-wheat", "Bread, Whole Wheat", "grain", 1050, 13, 43, 4.5, { region: "global", fiber: 7, aliases: ["wholewheat bread", "brown bread"] }),
  row("bread-rye", "Bread, Rye", "grain", 950, 9, 47, 1.5, { region: "global", fiber: 5 }),
  row("bread-sourdough", "Bread, Sourdough", "grain", 1e3, 11, 47, 2.5, { region: "global" }),
  row("pita", "Pita Bread", "grain", 1160, 9, 56, 1, { region: "global" }),
  ...
  row("pasta-cooked", "Pasta (boiled)", "grain", 580, 5, 31, 0.9, { region: "global", aliases: ["spaghetti", "macaroni", "penne", "noodles"] }),
  row("noodles-egg", "Egg Noodles (boiled)", "grain", 600, 5, 27, 1.5, { region: "global" }),
  ...
  row("couscous-cooked", "Couscous (boiled)", "grain", 550, 4, 25, 0.7, { region: "global" }),
  ...
  row("margarine", "Margarine / Brick Fat", "fat", 3e3, 0, 0, 80, { region: "sa", aliases: ["brick margarine", "stork"] }),
  ...
  row("biscuits-oreo", "Oreo Cookie", "snack", 2e3, 5, 70, 20, { region: "branded", aliases: ["oreo", "oreos"] }),
  ...
  row("biltong", "Biltong (dried beef, lean)", "protein", 1e3, 50, 1, 5, { region: "sa", aliases: ["biltong"] }),
  ...
  row("beyond-burger", "Beyond Burger Patty", "protein", 1e3, 18, 5, 17, { region: "branded", aliases: ["beyond", "beyond meat"] }),
  row("impossible-burger", "Impossible Burger Patty", "protein", 1e3, 19, 8, 14, { region: "branded" }),
  ...
  row("sun-dried-tomato", "Sun-Dried Tomatoes", "condiment", 1e3, 14, 55, 3, { region: "global" }),
  ...
  row("oat-milk", "Oat Milk", "beverage", 200, 1, 7, 1.5, { region: "global" }),
  ...
  row("burger-chicken", "Chicken Burger (grilled)", "fast-food", 1e3, 18, 35, 8, { region: "global" }),
  ...
  row("marmite", "Marmite (yeast extract)", "spread", 1e3, 25, 25, 0, { region: "global", aliases: ["yeast extract"] }),
  row("cream-cheese-philadelphia", "Cream Cheese (Philadelphia)", "dairy", 1500, 6, 4, 34, { region: "branded" }),
  ...
  row("guava", "Guava (raw)", "produce", 285, 2.6, 14, 1, { region: "global", aliases: ["guavas"] }),
  ...
  row("starfruit", "Star Fruit (raw)", "produce", 145, 1, 7, 0.4, { region: "global", aliases: ["carambola"] }),
  row("tamarind", "Tamarind (raw)", "produce", 1e3, 2.8, 63, 0.6, { region: "global" }),
  ...
  // ── VEGETABLES EXTRA ──
  row("okra", "Okra (raw)", "produce", 145, 2, 7, 0.2, { region: "global" }),
  row("parsnip", "Parsnip (raw)", "produce", 320, 1.5, 17, 0.3, { region: "global" }),
  row("turnip", "Turnip (raw)", "produce", 110, 1.5, 5, 0.2, { region: "global" }),
  row("radish", "Radish (raw)", "produce", 65, 0.7, 3.4, 0.1, { region: "global" }),
  row("leek", "Leek (raw)", "produce", 170, 1.5, 8, 0.3, { region: "global" }),
  ...
  row("collard-greens", "Collard Greens (raw)", "produce", 130, 3, 5.4, 0.5, { region: "global" }),
  row("mushroom-portobello", "Mushroom, Portobello (raw)", "produce", 95, 3, 3, 0.3, { region: "global" }),
  row("mushroom-shiitake", "Mushroom, Shiitake (dried)", "produce", 1300, 9.6, 75, 1, { region: "global" }),
  row("bamboo-shoots", "Bamboo Shoots (canned)", "produce", 90, 1.7, 3, 0.2, { region: "global" }),
  ...
  // ── SAUCES, STOCK, ETC. ──
  row("stock-chicken", "Chicken Stock", "condiment", 50, 1, 1, 0.5, { region: "global" }),
  row("stock-vegetable", "Vegetable Stock", "condiment", 30, 0.5, 2, 0.2, { region: "global" }),
  row("stock-beef", "Beef Stock", "condiment", 50, 1, 1, 0.4, { region: "global" }),
  ...
  row("cinnamon", "Cinnamon (ground)", "condiment", 1e3, 4, 80, 1, { region: "global" }),
  ...
  row("rice-milk", "Rice Milk", "beverage", 200, 0.4, 9, 1, { region: "global" }),
  row("cashew-milk", "Cashew Milk", "beverage", 100, 0.4, 1, 1.5, { region: "global" }),
  ...
  // ── SAUCES, HERBS EXTRA ──
  row("parsley", "Parsley (fresh)", "produce", 200, 3, 6, 0.8, { region: "global" }),
  ...
  row("kebab-meat", "Kebab Meat (mixed)", "protein", 1e3, 22, 0, 16, { region: "global" }),
  // ── INDIAN / PAKISTANI ──
  ...
  row("vegan-butter", "Vegan Butter / Margarine", "fat", 3e3, 0, 0, 80, { region: "global" }),
  // ── FRUIT JUICE / NECTAR ──
  ...
  // ── SANDWICHES / WRAPS ──
  row("sandwich-ham-cheese", "Ham & Cheese Sandwich", "fast-food", 1e3, 12, 30, 11, { region: "global" }),
  ...
  row("chocolate-bar-snickers", "Snickers Bar (50g)", "snack", 2e3, 8, 60, 24, { region: "branded" }),
  // ── WATER / DRINKS EXTRA ──
  ...
  row("electrolyte-powder", "Electrolyte Powder", "supplement", 1e3, 0, 50, 0, { region: "global" }),
  ...
  row("dried-apricots", "Apricots, Dried", "baked", 1e3, 3, 65, 0.5, { region: "global", fiber: 7 }),
  ...
  row("dried-prunes", "Prunes (dried plums)", "baked", 1e3, 2, 64, 0.2, { region: "global", aliases: ["prunes"] }),
  ...
  row("pita-pocket", "Pita Pocket (with filling)", "fast-food", 1e3, 12, 35, 9, { region: "global" }),
  ...
  row("roti-chicken", "Roti with Chicken", "fast-food", 1e3, 14, 35, 11, { region: "global" }),
  ...
  row("creme-fraiche", "Cr\xE8me Fra\xEEche", "dairy", 1400, 2.5, 3, 33, { region: "global" }),
  row("clotted-cream", "Clotted Cream", "dairy", 2e3, 2, 7, 50, { region: "global" }),
...
function searchFoods(query, limit = 10) {
  ...
  for (const food of FOOD_DB) {
    if (food.id === q) exactMatches.push({ food, score: 1e3 });
  ...
  const all = [...exactMatches, ...partialMatches, ...wordMatches].sort((a, b) => b.score - a.score).slice(0, limit).map((m) => m.food);
  return all;
}
...
// server/lifestyle/foodIdentifier.ts
init_db();
var GEMINI_API_KEY3 = process.env.GEMINI_API_KEY || "";
var DEEPSEEK_API_KEY2 = process.env.DEEPSEEK_API_KEY || "";
var IDENTIFICATION_PROMPT = `You are a nutrition assistant. Look at the image of food carefully.
...
async function identifyFoodFromImage(imageBase64) {
  ...
  if (GEMINI_API_KEY3) {
    try {
      ...
      console.warn("[FoodID] Gemini call failed:", e.message);
    }
  }
  if (DEEPSEEK_API_KEY2) {
    try {
      const result = await callDeepSeekVision(cleanBase64);
      if (result) return enrichWithDbMatch(result, "deepseek");
    } catch (e) {
      console.warn("[FoodID] DeepSeek call failed:", e.message);
    }
  }
  return {
    primary: {
    ...
    confidence: "low",
    source: "none",
    note: !GEMINI_API_KEY3 && !DEEPSEEK_API_KEY2 ? "No vision API keys available. Set GEMINI_API_KEY or DEEPSEEK_API_KEY in environment." : "Both vision APIs returned an error."
  };
}
async function callGeminiVision(imageBase64) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY3}`;
...
async function callDeepSeekVision(imageBase64) {
  const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
function enrichWithDbMatch(identified, source) {
  const primaryMatches = searchFoods(identified.name, 3);
  const primary = primaryMatches[0];
  const alternatesRaw = identified.alternates || [];
  const alternates = [];
  ...
  const confidence = primary ? "high" : alternatesRaw.length > 0 ? "medium" : "low";
  return {
    primary: {
      ...identified,
...
function logFoodMiss(foodName, userId, source = "calculator") {
  const db = getDb();
  if (!db) {
  ...
  try {
    ...
    console.warn(`[FoodMiss] log failed:`, e.message);
  }
}
function getFoodMisses(limit = 50) {
  const db = getDb();
  if (!db) return [];
  try {
    return db.prepare(`
...
async function calculateNutrition(foodName, grams, userId) {
  ...
  if (DEEPSEEK_API_KEY3) {
    try {
      ...
      console.warn("[KJCalc] DeepSeek call failed:", e.message);
    }
  } else {
    logFoodMiss(cleanedName, userId, "calculator");
  }
...
function fromDb(food, grams) {
  ...
  const totals = {
    ...
    fiber: food.fiber ? Math.round(food.fiber * scale * 10) / 10 : void 0
  };
  return {
    food: food.name,
...
async function callDeepSeekEstimate(foodName, grams) {
...
- Be honest. If you don't know the food, set confidence to "low" and use generic values.`;
  ...
  const per100g = {
    ...
    fiber: parsed.per100g.fiber ?? void 0
  };
  const scale = grams / 100;
  const totals = {
    ...
    fiber: per100g.fiber ? Math.round(per100g.fiber * scale * 10) / 10 : void 0
  };
  return {
    food: foodName,
    source: "deepseek",
...
function makeFallback(foodName, grams, reason) {
  return {
    food: foodName,
    source: "fallback",
    grams,
...
async function calculateMeal(items, userId) {
...
// server/lifestyle/dailySource.ts
init_db();
var DAILY_SOURCES = [
  { id: "tarot", name: "Tarot", description: "A card for the day, drawn from the major arcana.", emoji: "\u{1F0CF}", category: "divinatory" },
  { id: "iching", name: "I Ching", description: "A line from the ancient Chinese Book of Changes.", emoji: "\u262F\uFE0F", category: "divinatory" },
  { id: "rumi", name: "Rumi", description: "A verse from the 13th-century Sufi poet.", emoji: "\u{1F339}", category: "spiritual" },
  { id: "sufi", name: "Sufi Wisdom", description: "A saying from the Sufi tradition \u2014 Hafiz, Rumi, others.", emoji: "\u{1F319}", category: "spiritual" },
  { id: "buddhist", name: "Buddhist", description: "From the Dhammapada, Thich Nhat Hanh, and the Buddha.", emoji: "\u{1FAB7}", category: "spiritual" },
  { id: "stoic", name: "Stoic", description: "Marcus Aurelius, Seneca, Epictetus \u2014 for the steady mind.", emoji: "\u{1F3DB}\uFE0F", category: "philosophical" },
  { id: "psalms", name: "Psalms (Universal)", description: "Hebrew wisdom, paraphrased for everyone, no specific belief.", emoji: "\u{1F54A}\uFE0F", category: "spiritual" },
  { id: "secular", name: "Secular Wisdom", description: "Khalil Gibran, Carl Sagan, Mary Oliver, and others.", emoji: "\u2728", category: "secular" }
];
var SOURCE_BY_ID = DAILY_SOURCES.reduce(
  (acc, s) => ({ ...acc, [s.id]: s }),
  {}
);
var ENTRIES = {
...
function getDailyInsightForSource(source, dateISO) {
  const today = dateISO || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
...
function getUserDailySource(userId) {
  const db = getDb();
  if (!db) return "tarot";
  try {
    const row2 = db.prepare(
    ...
    return row2?.daily_source || "tarot";
  } catch {
    return "tarot";
  }
}
function setUserDailySource(userId, source) {
  const db = getDb();
  if (!db) return false;
  ...
  try {
    ...
    console.warn("setUserDailySource failed:", e.message);
    return false;
  }
}
...
var appRouter = router({
  ...
  lifestyle: router({
    ...
    // ── Meal Planner ──
    suggestMeals: publicProcedure.input(z.object({ ingredients: z.array(z.string()), diet: z.string().optional(), allergies: z.array(z.string()).optional(), maxTime: z.number().optional() })).query(async ({ input }) => {
      return suggestMealsFromIngredients(input.ingredients, { diet: input.diet, allergies: input.allergies, maxTime: input.maxTime });
    }),
    breakdownRecipe: publicProcedure.input(z.object({ recipe: z.string().min(10) })).query(async ({ input }) => {
      return breakdownRecipe(input.recipe);
    }),
    weeklyMenu: publicProcedure.input(z.object({ diet: z.string().optional(), allergies: z.array(z.string()).optional(), calories: z.number().optional(), cuisine: z.string().optional(), budget: z.enum(["low", "medium", "high"]).optional() })).query(async ({ input }) => {
    ...
    saveMealPlan: protectedProcedure.input(z.object({ menu: z.any(), shoppingList: z.any() })).mutation(async ({ input, ctx }) => {
    ...
    // paragraph.
    dailyAnchor: publicProcedure.input(z.object({ reading: z.string().optional() }).optional()).query(({ input }) => {
    ...
    dailyReflection: publicProcedure.input(z.object({
      verse: z.object({
        reference: z.string(),
        text: z.string(),
...
Write the reflection now. Just the paragraph, no headers, no labels.`;
    ...
    searchFoods: publicProcedure.input(z.object({
    ...
    getFood: publicProcedure.input(z.object({ id: z.string() })).query(({ input }) => {
      return getFoodById(input.id);
    }),
    // ── All foods in a category ──
    getFoodsByCategory: publicProcedure.input(z.object({ category: z.string() })).query(({ input }) => {
      return getFoodsByCategory(input.category);
    }),
    // ── Database stats (total items, SA staples, by-category breakdown) ──
    getFoodDbStats: publicProcedure.query(() => {
      return getDbStats();
    }),
    // ── Identify a food from a base64 image (Gemini vision → DB match) ──
    identifyFood: publicProcedure.input(z.object({
    ...
    calculateKJ: publicProcedure.input(z.object({
      foodName: z.string().min(1),
      grams: z.number().min(1).max(5e3)
    })).mutation(async ({ input, ctx }) => {
      const userId = ctx.user?.id;
      const result = await calculateNutrition(input.foodName, input.grams, userId);
      if (userId && result.source !== "fallback") {
        try {
          const dbConn = (await Promise.resolve().then(() => (init_db(), db_exports))).getDb();
          if (dbConn) {
            dbConn.prepare(
          ...
          console.warn("[KJCalc] save failed:", e.message);
        }
      }
      return result;
    }),
    // ── Calculate kJ for a whole meal (list of items) ──
    calculateMealKJ: publicProcedure.input(z.object({
    ...
    getFoodMisses: publicProcedure.input(z.object({ limit: z.number().min(1).max(200).default(50) })).query(({ input }) => {
      return getFoodMisses(input.limit);
    }),
    // ── User's own calculation history ──
    myFoodHistory: protectedProcedure.input(z.object({ limit: z.number().min(1).max(100).default(20) }).optional()).query(async ({ input, ctx }) => {
      const dbConn = (await Promise.resolve().then(() => (init_db(), db_exports))).getDb();
      if (!dbConn) return [];
      try {
...
- Do NOT add commentary outside the JSON.`;
    ...
    setMyDailySource: protectedProcedure.input(z.object({ source: z.string() })).mutation(({ input, ctx }) => {
    ...
    getDailyInsight: publicProcedure.input(z.object({
      source: z.string().optional(),
      reading: z.string().optional()
    }).optional()).query(({ input, ctx }) => {
      const source = input?.source || (ctx.user ? getUserDailySource(ctx.user.id) : "tarot");
